...
const userDelays = new Map(); // Tracks users who need delay before re-matching
...
function tryToMatchUsers() {
  while (waitingQueue.length >= 2) {
    ...
    // Check if users need delay before matching
    if (userDelays.has(user1) || userDelays.has(user2)) {
      console.log(`Delaying match for ${user1} or ${user2}`);
      // Put users back in queue and try later
      waitingQueue.unshift(user2);
      waitingQueue.unshift(user1);
      
      // Clear delays after current cycle
      setTimeout(() => {
        userDelays.delete(user1);
        userDelays.delete(user2);
      }, 3000);
...
// Proper disconnection handler with delay protection
function handleDisconnection(socketId, addToQueue) {
  ...
  if (partnerId) {
    ...
    if (partnerSocket) {
      ...
      // Add delay protection for the partner to prevent immediate re-matching
      userDelays.set(partnerId, Date.now());
      
      // Automatically add the partner to the queue to find a new connection (with delay)
      setTimeout(() => {
        if (!waitingQueue.includes(partnerId) && io.sockets.sockets.get(partnerId)) {
          waitingQueue.push(partnerId);
          partnerSocket.emit('waitingForPartner');
          console.log(`Partner ${partnerId} added to waiting queue after disconnection`);
          
          // Remove delay after adding to queue
          setTimeout(() => {
            userDelays.delete(partnerId);
            tryToMatchUsers();
          }, 1000);
        }
      }, 2000); // 2-second delay for older devices
    }
  }
  ...
  // Add back to queue if requested (with delay protection)
  if (addToQueue) {
    userDelays.set(socketId, Date.now());
    
    setTimeout(() => {
      if (!waitingQueue.includes(socketId) && io.sockets.sockets.get(socketId)) {
        waitingQueue.push(socketId);
        const userSocket = io.sockets.sockets.get(socketId);
        if (userSocket) userSocket.emit('waitingForPartner');
        
        // Remove delay and try matching
        setTimeout(() => {
          userDelays.delete(socketId);
          tryToMatchUsers();
        }, 1000);
