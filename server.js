...
io.on('connection', (socket) => {
  ...
  updateOnlineUsers(); // Broadcast updated count
  ...
  socket.on('nextPartner', () => {
    ...
    // Try to match immediately after disconnection
    tryToMatchUsers();
  });
...
function handleDisconnection(socketId, addToQueue) {
  ...
  if (partnerId) {
    ...
    if (partnerSocket) {
      ...
      // Automatically add the partner to the queue to find a new connection
